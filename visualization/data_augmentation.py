...
metric_l = ['SHD', 'TPR', 'FPR', 'F1', 'precision', 'recall']
for metric in metric_l:
    fig_path = f'../figure/data_augmentation/{metric}_augmentation.png'
    # 预先定义所有可能的 CIT 方法及其颜色映射
    fixed_cit_values = df['cit'].unique()
    augment_sample_generator_mask = df['sample_augment_method'] == 'gt_graph'
    df.loc[augment_sample_generator_mask, 'sample_augment_method'] = 'GT Sampling'
    augment_methods = ['GT Sampling', 'AdaSyn']
    ...
    for i, row in enumerate(row_index):
        ...
        for j, col in enumerate(col_index):
            ...
            for k, cit in enumerate(fixed_cit_values):
                ...
                sns.boxplot(x='Augment Size', y=metric, data=df_filter2, hue='sample_augment_method',
                            palette=augment_method_palette, ax=ax, legend=False, order=fixed_augment_sample_size,
                            showmeans=True, hue_order=augment_methods,
    ...
    fig.text(legend_bbox.x0 - 0.02, (legend_bbox.y0 + legend_bbox.y1) / 2, '数据增强方式：',
             fontsize=24,
             fontfamily=font_family,
             fontstyle=font_style,
             fontweight=font_weight,

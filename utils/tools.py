...
def substitute_i(p_array, idx):
    p_array_dc = p_array.copy()
    p_array_dc[idx] = np.minimum(p_array_dc[idx], 1 - p_array_dc[idx])
    return p_array_dc


def tilde_substitute_i(p_array, idx):
    p_array_dc = p_array.copy()

    # Convert input to a NumPy array to handle both lists and scalars
    p_array_dc = np.asarray(p_array_dc)
    # Element-wise minimum between p and 1-p
    tilde_p_array = np.minimum(p_array_dc, 1 - p_array_dc)
    # Invert the i-th element
    tilde_p_array[idx] = 0.0
    return tilde_p_array.tolist()
...
class WeightDistributionFunction:
    def __init__(self, distribution='uniform', random_state=42, **kwargs):
        self.kwargs = kwargs

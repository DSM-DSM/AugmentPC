...
def fdr_skeleton_discovery(data, uit, cit, hmt, alpha: float, stable: bool = True,
                           background_knowledge: BackgroundKnowledge | None = None, verbose: bool = False,
                           show_progress: bool = False, node_names: List[str] | None = None,
                           **kwargs) -> CausalGraphPlus:
    assert type(data) == np.ndarray
    assert 0 < alpha < 1
    ...
    while cg.max_degree() - 1 > depth:
        ...
        for x in range(no_of_var):
            ...
            for y in Neigh_x:
                ...
                for S in combinations(Neigh_x_noy, depth):
                    ...
                    p_dict[(x, y, S)] = p
                    # if len(S) == 0:
                    #     gt_dict[(x, y, S)] = float(nx.is_d_separator(uit.gt_graph, {x}, {y}, set(S)))
                    # else:
                    #     gt_dict[(x, y, S)] = float(nx.is_d_separator(cit.gt_graph, {x}, {y}, set(S)))

        # execute multiple test in order to control the FDR
        test_problems = [{
            'problem': eval(f"({sorted([x, y])[0]},{sorted([x, y])[1]},{S})"),
            'e': 1 / p if p > 0 else math.inf,
            'p': p,
            # 'reject_gt': bool(1 - gt_dict[(x, y, S)]),
        }
            for (x, y, S), p in p_dict.items()
        ]

        df_test = pd.DataFrame(test_problems)
        df_unique = df_test.drop_duplicates(subset=['problem'], keep='first')

        procedure = getattr(hmt, kwargs["procedure"])
        df_unique = df_unique.assign(
            rejection=procedure(df_unique['p'].values)[0],  # Using FDR Procedure
        )
        df_unique.set_index('problem', inplace=True)
        ...
        for x in range(no_of_var):
            ...
            for y in Neigh_x:
                ...
                for S in combinations(Neigh_x_noy, depth):
                    problem = eval(f"({sorted([x, y])[0]},{sorted([x, y])[1]},{S})")
                    p = df_unique.loc[df_unique.index == problem, 'p'].values[0]
                    rej = df_unique.loc[df_unique.index == problem, 'rejection'].values[0]
                    if not rej:
                        if verbose:
                            logging.info('%d ind %d | %s with p-value %f\n' % (x, y, S, p))
                        if not stable:
